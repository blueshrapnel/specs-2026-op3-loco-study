# Nicola's review — NCV2

Source: `specs_2026_op3_loco_study_NCV2.pdf` (60 annotations across 2 pages).
...
- **p2 · comment** — “The current pipeline is: MuJoCo MJCF → MJX → Brax PPO → ONNX → ROS 2 → Webots / OP3.”
  > I would move this at the end of the paragraph, where the meaning of all these steps will be clearer.

- **p2 · sticky note** — “”
  > explain better the role of sim-to-sim, as preliminary test before moving to sim-to-real. 
  > I can see that this is well explained at the beginning of the next section, but it should be said clearly the first time the sim-to-sim is mentioned in the paper.

- **p2 · comment** — “MJCF model,”
  > ?

- **p2 · comment** — “This model is executed in MJX for accelerated physics simulation, enabling large-scale reinforcement learning with PPO implemented in Brax.”
  > this sentence would have partially helped in the previous page, where these acronyms were introduced without defining them ("partially" because I still think it is not enough, you want to be even clearer in this regard)

- **p2 · comment** — “MuJoCo Menagerie”
  > ?

- **p2 · comment** — “uns”
  > "runs" where?

- **p2 · insertion / replacement** — “n”
  > "space"
  >
  > obtained how? From the robot camera?

- **p2 · comment** — “To ensure that the same policy can be deployed across simulators and hardware without modifying the underlying robot description, we introduce a bidirectional mapping layer, termed Flip–Flop. The Flip component transforms observations from the target system (Webots or physical OP3) into the convention expected by the policy. The Flop component performs the inverse transformation on the policy outputs, mapping joint-position commands into the correct target-system actuator ordering.”
  > Is this really relevant? It sounds a too specific implementation detail. I think this should be removed or moved to the appendix

- **p2 · comment** — “20-dimensional joint-positio”
  > and how many DOF has the robot? Maybe a brief description of this kind would help to appreciate more these values

- **p2 · comment** — “default pose.”
  > what pose is the default pose?

- **p2 · comment** — “to enable deployment outside the JAX runtime”
  > without providing more context, this is not clear

- **p2 · comment** — “inference”
  > I am not sure I understand the use of the word "inference" here
